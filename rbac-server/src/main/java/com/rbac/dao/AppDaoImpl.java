...
public class AppDaoImpl implements AppDao {
    ...
    public App createApp(final App app) {
        final String sql = "insert into sys_app(name, app_key, app_secret, available) value (?,?,?,?)";
    ...
    public App findOne(Long appId) {
        ...
        List<App> appList = jdbcTemplate.query(sql, new BeanPropertyRowMapper<App>(), appId);
        if (appList.size() == 0) {
            return null;
        }
